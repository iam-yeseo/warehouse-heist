...
- `node tests/mobile-state.cjs`: 모바일 시야·충돌 영역, 회전 시 공중 높이와 장애물 위치 유지, 남은 거리·시간과 부스트 상태 확인.
...
- 상단 첫 줄에 스테이지·생명·부스트 충전 상태, 둘째 줄에 남은 거리·시간을 표시합니다. 거리는 기존 30초 스테이지 진행률을 300m 코스로 환산한 값이며 차량 속도의 적분값은 아닙니다.
